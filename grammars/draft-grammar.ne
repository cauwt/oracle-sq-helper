...
{%
    d => {let v= [d[0], ...d[1].map(item => item[3])];
		 return {type:'select_list',
				offset:v[0].offset,
				line:v[0].line,
				col:v[0].col,
				text:[v.map(item => item.text)].join(''),
				select_items:v
				}
		  ;
		 }
...
kw_not -> %KW_NOT{%([d]) => toKeyword(d)%}
kw_null -> %KW_NULL{%([d]) => toKeyword(d)%}
kw_true -> %KW_TRUE{%([d]) => toKeyword(d)%}
